export default (
	{ LitElement, css, html, unsafeHTML, until }, // Pass Lit as parameter so as to not repeatedely import when not using a bundler
	getSentencesAsXML,
	dynamicallyImportComponents = true, // When not using a bundler, import components dynamically to speed up initial site loading time
) => customElements.define('ixalang-entries', class IxalangEntries extends LitElement {
	...
	async #handleClickEvent(e) {
		...
		if (dynamicallyImportComponents && customElements.get('ixalang-sentences') === undefined) {
			console.log('Importing ixalang sentences lit element...')
			const { default: loadIxalangSentences } = await import('./ixalang-sentences.js')
			loadIxalangSentences({ LitElement, css, html, unsafeHTML, until }, getSentencesAsXML)
		}
